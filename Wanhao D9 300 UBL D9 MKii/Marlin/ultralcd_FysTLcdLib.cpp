...
static void dwinExeCmd(millis_t& tNow)
{
    ...
    case MACRO_var_V068:
        ...
        break;
                ...
                {
                    SERIAL_ECHOLNPAIR("zprobe_zoffset comfirm:",zprobe_zoffset); 
                    do_probe_raise(5);
                    FunV007();
                    dwinSave(); 
                    #if FYSTLCD_PAGE_EXIST(MAIN)
    ...
    case MACRO_var_V026:
        ...
        else
        {
            ...
            case VARVAL_PRINT_SD_REFRESH:
                ...
                break;
            #if defined(MACRO_var_V010)&&FYSTLCD_PAGE_EXIST(MAIN)
            case MACRO_var_V03D:  
                #if FYSTLCD_PAGE_EXIST(MAIN)
                lcd_setPage(FTPAGE(MAIN));
                #endif
...
static void FunV046()
{
    ...
    for (char i = 0; i < 2; i++)
    {
        myFysTLcd.FunV031(lcd_preheat_fan_speed[i]);
    }
#if ENABLED(PID_EXTRUSION_SCALING)
...
static void FunV03A()
{
    ...
    if (myFysTLcd.FunV027(20))
    {
        ...
        for (char i = 0; i < 2; i++)
        {
            myFysTLcd.FunV03E(lcd_preheat_fan_speed[i]);
        }
    #if ENABLED(PID_EXTRUSION_SCALING)
...
void lcd_zstep_zoffset(bool dir) {
    ...
    else
    {
        ...
        SERIAL_ECHOLNPAIR("bi:",babystep_increment);
